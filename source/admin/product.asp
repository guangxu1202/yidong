...
'SortDiyField
ProArray = array("ProID","ProName","ProSort","ProsmallPic","ProBigPic","ProIsTop","ProIsReview","ProKeywords","Prodescription","ProContent")
if request.QueryString("act") = "add" then
	...
	eRs.open "select * from Products where 1<>1",eConn,1,3
	eRs.Addnew
	for i = 1 to ubound(ProArray)
...
elseif request.QueryString("act") = "plup" then
	...
	case "move"
		eConn.execute("update products set prosort = "&artsort&" where proid in("&plid&")")
	case "del"
		eConn.execute("update products set proisrec = true where proid in("&plid&")")
	case "top"
		response.Write("update products set proistop = true where proid in("&plid&")")
		eConn.execute("update products set proistop = true where proid in("&plid&")")
	case "untop"
		eConn.execute("update products set proistop = false where proid in("&plid&")")
	end select
	dbclose
	writealert "批量处理成功！","product.asp?page="&intpage&"&sortid="&sortid
elseif request.QueryString("act") = "del" then
	newsid = int(request.Form("id"))
	dbopen
	eConn.execute("update products set proisrec=true where proid = " & newsid)
	dbclose
	response.Write("0")
	response.End()
elseif request.QueryString("act") = "ups" then
	...
	eRs.open "select * from products where proid = " & newsid,eConn,1,3
	'eRs.Addnew
	for i = 1 to ubound(ProArray)
		'response.Write(ProArray(i) & ":")
		eRs(ProArray(i)) = ProArrayValue(i)
	...
	eRs.Close
	dbclose
	writealert "修改成功","product.asp?page="&intpage&"&sortid="&sortid
end if
...
if request.QueryString("act") = "up" then
	...
	eRs.open "select top 1 * from products where proid = " & newsid,eConn,0,1
		for i = 1 to ubound(ProArray)
	...
	<div id="wrapper">
        ...
        <div id="containerHolder">
			<div id="container">
                ...
                <h2><a href="#">网站内容管理</a> &raquo; <a href="#" class="active">产品(图片)管理</a></h2>
                ...
                <div id="main">
					...
					<h3>产品(图片)列表</h3>
                        ...
                        <a href="?">全部</a>&nbsp;&nbsp;
                        <%
						eRs.open "select sortid,sortname from sorts where sortid in (select proSort from products)",eConn,0,1
						...
						recordcounts = eConn.execute("select count(*) from products where proisrec=false " & newssql)(0)
						...
						if intpage = 1 then 
							newssql = "select top "&pagesizes&" a.*,sortname,b.sortshowtemplates from (products a left join sorts b on a.prosort = b.sortid) where  proisrec=false  " & newssql & " order by proistop asc,proid desc"
						else
							newssql = "select top "&pagesizes&" a.*,sortname,b.sortshowtemplates from (products a left join sorts b on a.prosort = b.sortid) where  proisrec=false and proid not in (select top "&(intpage-1)*pagesizes&" proid from products where  proisrec=false "&newssql&" order by proistop asc,proid desc) " & newssql & " order by proistop asc,proid desc"
						end if
						...
						%>
							<tr id="t<%=eRs("proid")%>" <%if i mod 2 = 0 then response.Write("class=""odd""") end if%>>
                                ...
                                <td class="action"><a href="../<%=tempshow%>?proid=<%=eRs("proid")%>" class="view" target="_blank">查看</a><a href="?act=up&sortid=<%=sortid%>&page=<%=intpage%>&id=<%=eRs("proid")%>" class="edit">修改</a><a href="javascript:void(0);" onclick="delnews(<%=eRs("proid")%>);" class="delete">删除</a></td>
                            </tr>  
                        <%
						eRs.movenext
						loop
                    	...
                    	<fieldset>
                            ...
                            <button type="button" onclick="window.location='product.asp';"><span><span>返 回</span></span><div></div></button>
                            <%end if%>
                        </fieldset>
                    </form>
                </div>
