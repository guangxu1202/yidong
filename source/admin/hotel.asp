...
'SortDiyField
ProArray = array("ProID","ProName","ProSort","ProsmallPic","ProBigPic","ProIsTop","ProIsReview","ProKeywords","Prodescription","ProContent","proStar","proPhone","proZone","proSub")
if request.QueryString("act") = "add" then
	...
	eRs.open "select * from hotel where 1<>1",eConn,1,3
	eRs.Addnew
	for i = 1 to ubound(ProArray)
...
elseif request.QueryString("act") = "plup" then
	...
	case "move"
		eConn.execute("update hotel set prosort = "&artsort&" where proid in("&plid&")")
	case "del"
		eConn.execute("update hotel set proisrec = true where proid in("&plid&")")
	case "top"
		response.Write("update hotel set proistop = true where proid in("&plid&")")
		eConn.execute("update hotel set proistop = true where proid in("&plid&")")
	case "untop"
		eConn.execute("update hotel set proistop = false where proid in("&plid&")")
	end select
	dbclose
	writealert "批量处理成功！","hotel.asp?page="&intpage&"&sortid="&sortid
elseif request.QueryString("act") = "del" then
	newsid = int(request.Form("id"))
	dbopen
	eConn.execute("update hotel set proisrec=true where proid = " & newsid)
	dbclose
	response.Write("0")
	response.End()
elseif request.QueryString("act") = "ups" then
	...
	eRs.open "select * from hotel where proid = " & newsid,eConn,1,3
	'eRs.Addnew
	for i = 1 to ubound(ProArray)
		'response.Write(ProArray(i) & ":")
		eRs(ProArray(i)) = ProArrayValue(i)
	...
	eRs.Close
	dbclose
	writealert "修改成功","hotel.asp?page="&intpage&"&sortid="&sortid
end if
...
if request.QueryString("act") = "up" then
	...
	eRs.open "select top 1 * from hotel where proid = " & newsid,eConn,0,1
		for i = 1 to ubound(ProArray)
	...
	<div id="wrapper">
        ...
        <div id="containerHolder">
			<div id="container">
                ...
                <h2><a href="#">网站内容管理</a> &raquo; <a href="#" class="active">酒店信息管理</a></h2>
                ...
                <div id="main">
					...
					<h3>酒店信息列表</h3>
                        ...
                        <a href="?">全部</a>&nbsp;&nbsp;
                        <%
						eRs.open "select sortid,sortname from sorts where sortid in (select proSort from hotel)",eConn,0,1
						...
						recordcounts = eConn.execute("select count(*) from hotel where proisrec=false " & newssql)(0)
						...
						if intpage = 1 then 
							newssql = "select top "&pagesizes&" a.*,sortname,b.sortshowtemplates from (hotel a left join sorts b on a.prosort = b.sortid) where  proisrec=false  " & newssql & " order by proistop asc,proid desc"
						else
							newssql = "select top "&pagesizes&" a.*,sortname,b.sortshowtemplates from (hotel a left join sorts b on a.prosort = b.sortid) where  proisrec=false and proid not in (select top "&(intpage-1)*pagesizes&" proid from hotel where  proisrec=false "&newssql&" order by proistop asc,proid desc) " & newssql & " order by proistop asc,proid desc"
						end if
						...
						%>
							<tr id="t<%=eRs("proid")%>" <%if i mod 2 = 0 then response.Write("class=""odd""") end if%>>
                                ...
                                <td class="action"><a href="../<%=tempshow%>?hproid=<%=eRs("proid")%>" class="view" target="_blank">查看</a><a href="?act=up&sortid=<%=sortid%>&page=<%=intpage%>&id=<%=eRs("proid")%>" class="edit">修改</a><a href="javascript:void(0);" onclick="delnews(<%=eRs("proid")%>);" class="delete">删除</a></td>
                            </tr>  
                        <%
						eRs.movenext
						loop
                    	...
                    	<fieldset>
							...
							<p><label>联系电话:</label><input type="text" name="proPhone" value="<%=ProArrayValue(11)%>" class="text-long" /></p>
							<p><label>酒店位置:</label><input type="text" name="proZone" value="<%=ProArrayValue(12)%>" class="text-long" /></p>
							<p><label>附属设施:</label><input type="text" name="proSub" value="<%=ProArrayValue(13)%>" class="text-long" /></p>
                            ...
                            <p><label style="position:relative;">产品小图:
