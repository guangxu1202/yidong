...
Sub FileUpload( resourceType, currentFolder, sCommand )
	...
	Else
		...
		if not (oFSO.FolderExists( sServerDir ) ) then
			...
			sFileName = GetNewFileName() & "." & sExtension
			sFileName = SanitizeFileName( sFileName )
			sFileName = replace(sFileName,"as","_as_")
...
Public Function GetNewFileName()
    ...
    dim tempFileName
    tempFileName=year(dtNow) & right("0" & month(dtNow),2) & right("0" & day(dtNow),2) & right("0" & hour(dtNow),2) & right("0" & minute(dtNow),2) & right("0" & second(dtNow),2) & ranNum
    tempFileName = replace(tempFileName,"asp","txt")
    tempFileName = replace(tempFileName,"aspx","txt")
    tempFileName = replace(tempFileName,"asa","txt")
    tempFileName = replace(tempFileName,"php","txt")
	tempFileName = replace(tempFileName,".","_")
    GetNewFileName=tempFileName
End Function
